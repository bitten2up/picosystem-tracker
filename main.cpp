...
std::array<std::array<dial_t, 13>, 128> dials;
...
void update(uint32_t tick) {
  ...
  if(tick % 10 == 0) {
    if(button(UP)) {
      adjust_dial_value(dials[curnote][selected], dials[curnote][selected].step);
      change = true;
    }
    if(button(DOWN)) {
      adjust_dial_value(dials[curnote][selected], -dials[curnote][selected].step);
  ...
  if (pressed(Y)) // start playing one by one
  {
  ...
  if ((tick-startTime) % get_dial_value("tempo") == 0) {
    if (songPlaying)
      curnote++;
  }
  ...
  // previous dial
  if(pressed(LEFT)) {
    selected = selected == 0 ? dials[curnote].size() - 1 : selected - 1;
  }
  if(pressed(RIGHT)) {
    selected = selected < dials[curnote].size() - 1 ? selected + 1 : 0;
  }
  if (pressed(A))
    curnote++;
  if (pressed(B))
